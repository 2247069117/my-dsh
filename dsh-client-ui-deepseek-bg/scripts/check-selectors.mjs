 ...
 *
 * 用法：
 *   node scripts/check-selectors.mjs          # 静态扫描：检查所有 src/css 与 src/*.js 中的选择器是否符合白名单与fallback规范
 *   node scripts/check-selectors.mjs --live   # （可选）若本机 3080 可用，尝试无头抓取并统计命中数
 *
 * 目前仅做静态规则检查，不依赖浏览器：
 *  - 每个哈希类名必须同时有 [data-*] 或通用后缀 fallback
 *  - 关键 data-* 选择器必须存在
 */
...
if (hashedInCss.length>0) {
  // 检查每个文件是否也包含对应的 [data-*] fallback（简单：全文是否含 data-slot 或 data-composer-card）
...
if (hashedInJs.length>0) {
  // 检查每个哈希查询附近 200 字符内是否有 data- 备选（仅警告，不阻断）
