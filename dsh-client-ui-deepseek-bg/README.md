...
- **渲染质量（高级）**：极光分辨率滑杆（0.4x–1.0x）、动画帧率上限（20/24/30fps；**鼠标交互期间自动提到 60fps** 保证跟手，停止移动 200ms 后回落所选帧率）、玻璃模糊强度（6/8/10/12px）、**低电量自动节能**（Battery API：电量 ≤20% 未充电时自动切「节能」，恢复后还原原档位）
