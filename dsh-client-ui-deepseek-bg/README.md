...
- **特效开关**：极光背景 / 粒子鲸鱼 / 星座网格 / **鼠标跟随交互**（极光/鲸鱼/星座跟随光标互动，跟手优化：输入与渲染解耦、flowmap 交互期 60Hz、帧率无关临界阻尼平滑 ~20ms 无输入滞后；关闭后极光改为**自主缓慢漂移**的漫游笔刷，画面保持流动、不再响应鼠标）/ Border Beam / 玻璃拟态 / Thinking Orbs 独立开关（带负载标签；手动调整后档位自动变为「自定义」）
- **渲染质量（高级）**：极光分辨率滑杆（0.4x–1.0x）、动画帧率上限（20/24/30fps；**鼠标交互期间自动提到 60fps** 保证跟手，停止移动 200ms 后回落所选帧率）、玻璃模糊强度（6/8/10/12px）、**跟手灵敏度**（平滑时间常数 5–120ms，默认 20ms：越小越贴手、越大越绵柔，作用于极光笔刷/拖尾与鲸鱼光线）、**光线跟随强度**（0–100%，默认 100%：0% 时光线固定不随鼠标）、**低电量自动节能**（Battery API：电量 ≤20% 未充电时自动切「节能」，恢复后还原原档位）
