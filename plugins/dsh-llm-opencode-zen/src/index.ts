...
// ---------------------------------------------------------------------------
// Config（模型字段一律不出现 → 模型只读，只能由自动探测维护）
// ---------------------------------------------------------------------------
 ...
 */
function reasoningWire(effort: unknown): Record<string, unknown> {
  switch (effort) {
    case "off":
...
function serializeRequest(options: any): Record<string, unknown> {
  const messages: any[] = []
  ...
  for (const message of options.messages) {
    ...
    // user / tool-result
    const toolResults = message.content.filter((block: any) => block.type === "tool-result")
    const text = flattenText(message.content)
...
function closeBlock(block: OpenBlock): any {
  switch (block.kind) {
    case "text":
...
function mapUsage(usage: any): Record<string, number> {
...
async function* translate(
  ...
  for await (const payload of payloads) {
    if (payload === "[DONE]") {
      ...
      const reason =
        finishReason === undefined && order.length === 0
          ? {
              kind: "error",
              failure: {
...
async function httpError(baseURL: string, response: Response): Promise<LlmError> {
  let message = `OpenCode Zen API error (HTTP ${response.status})`
  let providerError: any
  try {
    providerError = ((await response.json()) as any)?.error
  } catch {
    // 非 JSON 错误体不致命
  }
  if (providerError?.message) message = String(providerError.message)
  ...
  else if (response.status === 413) code = "INVALID_REQUEST"
  else if (isQuotaExceededError(detail)) code = QUOTA_EXCEEDED_CODE
  ...
  else code = `HTTP_${response.status}`
...
interface ZenAdapterDeps {
  getSnapshot: () => ZenSnapshot
  getCache: () => any // core.ts Cache
}

class OpenCodeZenAdapter extends LlmAdapter {
  private deps: ZenAdapterDeps

  constructor(deps: ZenAdapterDeps) {
    super()
    this.deps = deps
  }
  ...
  listModels(provider: string): Promise<any[]> {
    ...
    const models = Object.values(cache.models ?? {}).map((meta: any) => ({
      provider,
      id: meta.id,
      name: meta.name,
  ...
  resolveModel(provider: string, model: string, _signal?: AbortSignal): Promise<any> {
  ...
  prepareCall(provider: string, model: string, signal?: AbortSignal): Promise<any> {
    return this.resolveModel(provider, model, signal).then((resolved: any) => ({
      model: resolved,
      stream: (options: any) => this.streamWith(options, this.deps.getSnapshot()),
    }))
  }
  ...
  async *streamWith(options: any, snap: ZenSnapshot): AsyncGenerator<any> {
    const signal: AbortSignal | undefined = options.signal
    try {
      if (options.messages.some((message: any) => contentHasImage(message.content))) {
        throw new LlmError(
          "OpenCode Zen free models do not accept image input.",
          "UNSUPPORTED_CONTENT",
        )
      }
      ...
      if (!response.ok) throw await httpError(snap.baseURL, response)
...
export function apply(ctx: any, entry: Config): void {
  let current = () => entry
  let lastRaw: unknown
  let lastGood: any
  ...
  const sync = (reason: string): Promise<void> => {
      ...
      try {
        ...
        const change =
          result.added.length > 0 || result.removed.length > 0
            ...
            : "no change"
        ctx.logger.info(
          `[opencode-zen] sync (${reason}): ${result.cache.models ? Object.keys(result.cache.models).length : 0} models, ${change}`,
        )
      } catch (error) {
  ...
  const ensureDirectory = () => {
    ...
    if (entries.length === 0) {
      if (directory !== undefined) directory.replace([])
      directoryKey = key
      return
    }
  ...
  const ensureRegistration = () => {
    ...
    if (routes.length === 0) {
      ...
      if (registration !== undefined) registration.replace([])
      routesKey = key
      return
    }
