...
function installOverlayInteractions(ctx) {
    installMobileEffect(ctx, 'dsh-mobile-nav: drawer close (Escape + navigate)', () => {
        ...
        const onDrawerPointerUp = (event) => {
            ...
            // are not treeitems): close only AFTER the browser's synthesized click
            // has fired. Toggling here at pointerup collapapsed the drawer (and a
            // co-installed compat plugin sets pointer-events:none on the collapsed
            // sidebar) before the click was dispatched, so the click re-hit-tested
            // onto whatever now covered the button and the target's onClick never
            // ran. The capture-phase onDrawerClick below closes the drawer first
            // on a well-behaved tap; this rAF*2 fallback only fires if it did not.
            const navAt = performance.now();
            lastTouchNavAt = navAt;
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    if (!drawerOpen())
                        return;
                    if (performance.now() - lastTouchNavAt > 700)
                        return;
                    toggleSidebar();
                });
            });
...
const phone_chrome_ts_1 = require("./effects/phone-chrome.js");
/**
 * Mobile-only Files action next to the session title:
...
function MobileNavToggle({ t }) {
    ...
    return ((0, jsx_runtime_1.jsx)("button", { type: "button", "data-mobile-nav": "files", "aria-label": t('files'), title: t('files'), onClick: toggleExplorer, children: (0, jsx_runtime_1.jsx)(dsh_client_ui_primitives_1.IconFolderOpenOutline16, { size: 16 }) }));
}
};
...
@media (max-width: 1023px) {
  ...
  [data-mobile-nav="frame"] > :first-child {
    ...
    width: max-content;
    max-width: 92vw;
    /* No z-index here: any plain z-index can lose to another plugin's
       full-screen backdrop pseudo-element on the frame (e.g.
       @linxin666/dsh-web-ui-all's ::after at z-index 1050). If the drawer
       sinks below that overlay, every tap inside the drawer hit-tests to
       the frame itself and never reaches the buttons. The open-state rule
       below lifts the drawer above all known overlays instead. */
  ...
  [data-mobile-nav="frame"]:not([data-sidebar-collapsed]) > :first-child {
    ...
    /* Sit above the co-installed ::after backdrop (z-index 1050) that
       would otherwise swallow taps aimed at the drawer's buttons, but
       BELOW the portaled per-session menus (z-index 1100) so an open
       menu stays visible and clickable above the drawer.
       Only the expanded state gets this: the collapsed rail must stay
       below full-screen takeover panels. */
...
/* ===== dsh-remote-web-ui 竖屏适配对冲（与 @linxin666/dsh-web-all 共存） =====
   该插件的移动层面向原生 shell，与本插件的抽屉布局冲突（两处均已 CDP 复现）：
   1. "compact picker" 在工具道注入两个图标按钮（#dshRemoteModelPick /
      #dshRemoteEffortPick），其点击回调 drillIntoPicker() 依赖
      _triggerEffort 子元素 —— 当前宿主已不再渲染 —— 两个按钮完全无响应；
      同时 body.dsh-remote-compact-picker 把 trailing 压到 width:0，官方
      模型胶囊被裁成 ~12px。客户端任务持续摘掉该 body class 并隐藏按钮，
      这里再以 CSS 兜底恢复 trailing 宽度。
   2. 鲸鱼按钮 #dshRemoteWhale 与本插件的 FAB / 侧栏切换重复，且其切换
      路径已被客户端守护拦截成死按钮，直接隐藏。
   JS 侧见 src/client/effects/remote-compat.ts（含设置抽屉塌陷的修复）。 */
@media (max-width: 1023px) {
  /* 死掉的 compact picker 按钮：隐藏（600ms 同步循环会重建 DOM，靠 CSS 持续生效）。 */
  #dshRemoteModelPick,
  #dshRemoteEffortPick {
    display: none !important;
  }
  /* 兜底：即便 class 短暂回归，也绝不压缩 trailing 车道 —— 官方模型胶囊
     必须保持可点。逐项反转 dsh-remote-web-ui 的 compact 规则。 */
  body.dsh-remote-compact-picker [class$="_composerSeat"] [class$="_trailing"] {
  ...
  /* dsh-remote-web-ui 对 trailing 的常驻规则（flex-basis:100% 会把尾部
     车道挤成独立一行、38/78px 内边距挤压模型胶囊）也一并反转，保证本
     插件的单行 composer 布局成立。 */
  [data-mobile-nav="frame"] [class$="_composerSeat"] [class$="_trailing"] {
    flex-basis: auto !important;
    padding-left: 0 !important;
    padding-right: 0 !important;
    position: static !important;
  }
  /* dsh-remote-web-ui 把发送按钮（_primary，right:8）与上下文计量器
     （含 _track 的 _root，right:52）绝对定位到 composer row 右缘 ——
     row 本身是 position:relative —— 两者的定位基准是整行而非 trailing
     车道，于是直接压在模型胶囊上（发送按钮 x≈323-357、计量器 x≈285-313，
     胶囊 x≈281-357）。本插件从未声明 position，remote 的绝对定位因此
     一直胜出。恢复常规流：让 [模型胶囊][上下文计量器][发送] 依次排在
     trailing 车道内（trailing 自身由上方规则恢复常规流并右对齐）。
     计量器锚定同时覆盖 :has(_track) 与 dialog 触发器两种识别方式，
     并用后代选择器兼容其可能套 display:contents 包装的情况。 */
  [data-mobile-nav="frame"] [class$="_composerSeat"] [class$="_trailing"] [class$="_root"]:has([class$="_track"]),
  [data-mobile-nav="frame"] [class$="_composerSeat"] [class$="_trailing"] [class$="_root"]:has(> [class*="_trigger"][aria-haspopup="dialog"]),
  [data-mobile-nav="frame"] [class$="_composerSeat"] [class$="_primary"] {
    position: static !important;
    right: auto !important;
    top: auto !important;
    transform: none !important;
...
exports.MISC_CSS = `@media (max-width: 1023px) {
  ...
  /* ---------- hide the floating "exit harness" power button on mobile ----------
     @linxin666/dsh-desktop-launcher mounts a fixed bottom-right power button
     ([data-dsh-shutdown-float]) that can end the dsh web process with one
     stray tap — risky on a touch device. Keep the desktop control, hide it on
     mobile. */
  [data-dsh-shutdown-float] {
    display: none !important;
  }
}
...
__modules["effects/remote-compat.js"] = function (require, module, exports) {
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.installRemoteCompat = installRemoteCompat;
const phone_chrome_ts_1 = require("./effects/phone-chrome.js");
/**
 ...
 */
function installRemoteCompat(ctx) {
    (0, phone_chrome_ts_1.installMobileEffect)(ctx, 'dsh-mobile-nav: dsh-remote-web-ui compat', () => {
        const w = window;
        ...
        const wrapAdapt = () => {
            ...
            if (adapt === undefined || adapt.__mobileNavWrapped === true)
                return;
        ...
        const onCaptureClick = (event) => {
            ...
            if (!(target instanceof Element))
                return;
            if (target.closest(FALLBACK_TOGGLE) === null)
                return;
        ...
        const removeTask = (0, phone_chrome_ts_1.addReconcilerTask)(task);
        task.ensure();
        return () => {
            removeTask();
...
const aionui_compat_ts_1 = require("./effects/aionui-compat.js");
const remote_compat_ts_1 = require("./effects/remote-compat.js");
...
function apply(ctx) {
    ...
    // Neutralize @linxin666/dsh-remote-web-ui's competing portrait layer: its
    // dead compact-picker composer buttons and its capture-phase handler that
    // collapses the drawer whenever Settings (portaled into the sidebar's
    // settings area) is used.
    (0, remote_compat_ts_1.installRemoteCompat)(ctx);
