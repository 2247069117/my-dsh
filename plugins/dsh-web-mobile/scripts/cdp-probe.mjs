...
// The directory toggle is host-owned in the logo row. The plugin only adds
// the Files action in the conversation header; a second plugin-owned toggle
// would duplicate the host control on narrow screens.
const MOBILE_TOGGLE_SELECTOR = '[data-dsh-responsive-part="sidebar-toggle"]';
const MOBILE_PLUGIN_TOGGLE_SELECTOR = '[data-mobile-nav="toggle"]';
...
async function runCoreScenario(client, config, signal, pageErrors) {
  ...
  const pluginToggleCount = await client.evaluate(`document.querySelectorAll(${JSON.stringify(MOBILE_PLUGIN_TOGGLE_SELECTOR)}).length`);
  check('mobile.no-duplicate-drawer-toggle', pluginToggleCount === 0, `pluginToggleCount=${pluginToggleCount}`);
